...
class ProfilingMyDion(Optimizer):
    """
                ...
                for params in create_param_batches(split_params, batch_size):
                    ...
                    yield AsyncTask(
                        dion_update_func(
                            ...
                            prob=self.prob,
                            step=step,
                            profiling_step=self._profiling_step,
                            n_power_iterations=self._iters
                        )
                    )
...
def dion_update_ddp(
    ...
    # modified
    torch._foreach_mul_(M, mu) # M = M * beta
    torch._foreach_add_(M, G) # M = M + G
    ...
    # <---  开始幂迭代循环
    # 如果 n_power_iterations=1，行为与原始代码完全一致
    # 如果 n_power_iterations>1，则会进行多次 M@Q -> P, M.T@P -> R
    ...
    for i in range(n_power_iterations):
        # -----------------------------------------------------------
        # Step A: Left Projection (P = M @ Q)
        # -----------------------------------------------------------
        # M_batch shape is (batch_size, m, n)
        # Q_batch shape is (batch_size, n, r) -> Q is updated in loop if iter > 0
        ...
        # -----------------------------------------------------------
        # Step B: Right Projection (R = M.T @ P)
        # -----------------------------------------------------------
        ...
        # NaN check (Important to do inside loop to avoid propagating NaNs)
        P_batch, R_batch = fix_all_zero_or_nan(P_batch, R_batch, Q_batch, M_batch)

        # -----------------------------------------------------------
        # Step C: Prepare Q for next iteration (if needed)
        # -----------------------------------------------------------
        # 如果这不是最后一次迭代，我们需要更新 Q_batch 以便下一轮计算更精准的 P
        if i < n_power_iterations - 1:
            Q_batch = column_normalize(R_batch, epsilon=epsilon)

    # <---  循环结束，此时 P_batch 和 R_batch 是经过 n 次迭代后的高精度近似结果
    
    # modified
    random_number = random.random()
    if random_number > prob:
        # X = (1 - lr * weight_decay) * X - scaled_lr * (M - P @ R.T)
        M_norm_sq = torch.linalg.vector_norm(M_batch, ord=2, dim=(-2, -1)) ** 2 # Shape: (Batch,)
        R_norm_sq = torch.linalg.vector_norm(R_batch, ord=2, dim=(-2, -1)) ** 2
        O_norm = torch.sqrt((M_norm_sq - R_norm_sq).clamp(min=1e-6))
        numel = X[0].numel()
        target_rms = 0.2
        norm_scale_factor = (target_rms * math.sqrt(numel)) / (O_norm + 1e-8)
        scaled_lr = lr * norm_scale_factor
        if weight_decay > 0:
            torch._foreach_mul_(X, 1 - lr * weight_decay)
        scaled_lr_list = scaled_lr.unbind(dim=0)
        scaled_M = torch._foreach_mul(M, scaled_lr_list)
        torch._foreach_sub_(X, scaled_M) #  X = X - scaled_lr * M
        foreach_baddbmm_(
            X,
            P_batch, 
            R_batch,  
            alpha=scaled_lr,  # positive, for scaled_lr + P @ R.T
            beta=1.0,
            transpose=param_config.is_transposed,
        )
    else:
        # X = (1 - lr * weight_decay) * X - scaled_lr * M
        M_norm = torch.linalg.vector_norm(M_batch, ord=2, dim=(-2, -1))
        numel = X[0].numel()
        target_rms = 0.2
        norm_scale_factor = (target_rms * math.sqrt(numel)) / (M_norm + 1e-8)
        scaled_lr = lr * norm_scale_factor
        if weight_decay > 0:
            torch._foreach_mul_(X, 1 - lr * weight_decay)
        scaled_lr_list = scaled_lr.unbind(dim=0)
        scaled_M = torch._foreach_mul(M, scaled_lr_list)
        torch._foreach_sub_(X, scaled_M)
    ...
    Q_batch = column_normalize(R_batch, epsilon=epsilon)
    if step % profiling_step == 0:
        _, m, n = M_batch.shape
        r = min(math.ceil(0.015 * min(m, n)), m, n)
        print(f"M_batc: m = {m}, n = {n}")
        M_pro = M_batch[0]
        Q_pro = Q_batch[0] # right
        P_pro = P_single[0] # left
        R_pro = R_batch[0] # W
        assert M_pro.shape == (m, n), f"M_pro shape should be {(m, n)}, but got {M_pro.shape}"
        assert Q_pro.shape == (n, r), f"Q_pro shape should be {(n, r)}, but got {Q_pro.shape}"
        assert P_pro.shape == (m, r), f"P_pro shape should be {(m, r)}, but got {P_pro.shape}"
        assert R_pro.shape == (n, r), f"R_pro shape should be {(n, r)}, but got {R_pro.shape}"
        U1, S1, V1h = torch.linalg.svd(M_pro, full_matrices=True)
        print(S1.tolist())
        V1 = V1h.T
        U1 = U1[:, :r]
        S1 = S1[:r]
        ...
        U2, S2, V2 = torch.svd_lowrank(M_pro, q=r, niter=2)
        # print(f"U2 shape: {U2.shape}, S2 shape: {S2.shape}, V2 shape: {V2.shape}")
        U3 = P_pro
        R_gram = R_pro.T @ R_pro  # (r, r)
        S3 = torch.sqrt(torch.diag(R_gram))
        V3 = Q_pro
        # print(f"U3 shape: {U3.shape}, S3 shape: {S3.shape}, V3 shape: {V3.shape}")
        def cosine_similarity_columns(A, B):
            # 1. 归一化 (L2 Normalize along dim=0)
            # A_norm = torch.nn.functional.normalize(A, dim=0)
            # B_norm = torch.nn.functional.normalize(B, dim=0)
            A_norm = A
            B_norm = B
            
            # 2. 计算全量相似度矩阵 (Correlation Matrix)
            # 结果 shape: (r, r)
            # sim_matrix[i, j] 代表 A 的第 i 列与 B 的第 j 列的点积
            sim_matrix = torch.mm(A_norm.T, B_norm)
            
            # 3. 取绝对值 (忽略方向翻转的影响)
            abs_sim_matrix = torch.abs(sim_matrix)
            
            # 4. 对于 B 中的每一列 (approx)，在 A 中找到最相似的一列 (ground truth)
            # dim=0 表示沿着行搜索最大值（即固定 B 的列 j，遍历 A 的所有列 i）
            # values shape: (r,)
            max_sims, _ = torch.max(abs_sim_matrix, dim=0)
            print(max_sims.tolist())
            
            # 5. 统计指标
            return max_sims.mean().item(), max_sims.max().item(), max_sims.min().item()

        # 1. 奇异值误差 (S)
        err_s_lowrank = torch.dist(S1, S2).item()
        err_s_dion    = torch.dist(S1, S3).item()

        # 2. 奇异向量相似度 (U 和 V) - 越接近 1 越好
        sim_u_lowrank, sul_max, sul_min = cosine_similarity_columns(U1, U2)
        sim_v_lowrank, svl_max, svl_min = cosine_similarity_columns(V1, V2)

        sim_u_dion, sud_max, sud_min = cosine_similarity_columns(U1, U3)
        sim_v_dion, svd_max, svd_min = cosine_similarity_columns(V1, V3)

        print(f"精度验证 (与 Full SVD 比较):")
        print(f"[Singular Values Error (L2 norm)]")
        print(f"  Lowrank: {err_s_lowrank:.4f}")
        print(f"  DION   : {err_s_dion:.4f}")

        print(f"\n[Vector Similarity (Avg Cosine Sim, 1.0 is best)]")
        print(f"  Lowrank: U_sim={sim_u_lowrank:.4f}, V_sim={sim_v_lowrank:.4f}, su_max={sul_max:.4f}, su_min={sul_min:.4f}")
        print(f"  DION   : U_sim={sim_u_dion:.4f},    V_sim={sim_v_dion:.4f}, sv_max={svd_max:.4f}, sv_min={svd_min:.4f}")
...
def foreach_baddbmm_(
    ...
    # alpha: float = 1.0,
    alpha: Union[float, Tensor] = 1.0, # 修改：支持 Tensor
    beta: float = 1.0,
    ...
    # 如果 alpha 是 Tensor (Batch,)，unbind 把它变成标量列表，才能传给 foreach_mul
    if isinstance(alpha, torch.Tensor) and alpha.ndim > 0:
        # 确保 alpha 是对应每个矩阵的标量列表
        alpha_list = alpha.unbind(dim=0)
        update = torch._foreach_mul(update, alpha_list)
    else:
